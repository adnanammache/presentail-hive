...
- **Agent workspace / Inbox**: conversations with every agent (start as many as you like; each is private to whoever started it, plus workspace owners, unless shared). Replies render as Markdown, the agent's tool activity shows as it works, and a Work overview beside the chat lists the conversation's task, what's coming up and recent files. Claude agents reply live. Webhook agents can reply synchronously or later through the API.
  - **Conversation history** beside the chat: search, Active / Archived / All, grouped by date, unread counts and what's waiting in each conversation, rename, share, archive and restore from each row.
  - **Side panel**: open a file (PDF, image, text, Markdown, CSV, JSON previews; other formats show details and Download), a task (the usual task details) or the Work overview beside the conversation, resizable, several at once as tabs. Select a passage (or the whole file) and *Discuss* it: a reference goes into your draft, and is sent only when you send it.
  - **Status strip** above the conversation when there's real work: a run working (with Stop for that run only), an action waiting for approval, a task waiting for your input or review (Approve / Request changes for the version you're looking at). Nothing shows when idle.
  - **Archiving** is personal: it tidies your history without touching the conversation, its tasks, runs or schedules, or anyone else's view. Undo, Restore, and links to archived conversations all work. See DEPLOY.md → Conversation history and archiving.
